    ...
    for (science_ccd_hdulist, filename) in science_images:
        try:
            ...
            try:
                ...
                aligned_data, _ = aa.register(corrected_ccd.data, reference_image.data, max_control_points=100, detection_sigma=0.5, min_area=3)
...
def non_local_means_denoise(image):
    ...
    sigma_est = np.mean(estimate_sigma(image, channel_axis=None))  # Use channel_axis instead of multichannel
    patch_kw = dict(patch_size=5, patch_distance=6, channel_axis=None)  # Ensure channel_axis is set
...
def display_and_save_images(stacked_with_bg_sub, output_folder):
    ...
    norm_asinh_denoised = ImageNormalize(nlm_denoised_data, stretch=AsinhStretch(a=0.03))  # Adjust the intensity of AsinhStretch
    data_asinh_denoised = norm_asinh_denoised(nlm_denoised_data)
    
    # Asinh Stretched Image on Denoised Data
    norm_asinh_denoised2 = ImageNormalize(nlm_denoised_data_only, stretch=AsinhStretch(a=0.03))  # Adjust the intensity of AsinhStretch
    data_asinh_denoised2 = norm_asinh_denoised2(nlm_denoised_data_only)

    # Apply CLAHE to Asinh Stretched Data
    #data_asinh_denoised_clahe = apply_clahe(data_asinh_denoised, clip_limit=0.5, tile_grid_size=(12, 12))

    # Log Stretched Image on Denoised Data
    norm_log_denoised = ImageNormalize(nlm_denoised_data, stretch=LogStretch(a=30))  # Adjust the intensity of LogStretch
    data_log_denoised = norm_log_denoised(nlm_denoised_data)

    # Apply CLAHE to Log Stretched Data with more conservative parameters
    #data_log_denoised_clahe = apply_clahe(data_log_denoised, clip_limit=0.03, tile_grid_size=(16, 16))

    # Apply Unsharp Masking to CLAHE applied images with more conservative parameters
    #ata_asinh_denoised_sharpened = unsharp_mask(data_asinh_denoised, amount=1.5)
    #data_log_denoised_sharpened = unsharp_mask(data_log_denoised, amount=1.5)

    # Deblurring using Richardson-Lucy deconvolution
    psf1 = np.ones((5, 5)) / 25  # Example PSF, adjust as needed
    psf2 = np.ones((7, 7)) / 49  # Example PSF, adjust as needed
    ...
    images = [
        ...
        #(data_asinh_denoised, 'Denoised + Asinh Stretched + CLAHE + Sharpened'),
        (data_log_denoised, 'wavelet_and_nonlocalmean_Denoised + Log Stretched'),
        #(data_log_denoised, 'Denoised + Log Stretched + CLAHE + Sharpened'),
